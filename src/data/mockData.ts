  ...
  for (let i = 1; i <= count; i++) {
    ...
    rounds.push({
      ...
      events: [
        {
          ...
          type: 'kill',
          player_id: mockPlayers[Math.floor(Math.random() * 5)].id,
          target_id: 'enemy_1',
      ...
      mistakes: Math.random() > 0.7 ? [{
        id: `m_${i}`,
        player_id: mockPlayers[Math.floor(Math.random() * 5)].id,
        player_name: mockPlayers[Math.floor(Math.random() * 5)].name,
        type: ['positioning', 'utility', 'timing', 'rotation'][Math.floor(Math.random() * 4)],
